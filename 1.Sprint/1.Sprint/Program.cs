...
namespace _1.Sprint
{
    class Program
    {
        static void Main(string[] args)     //Hauptprogramm
        {
            ...
            string gewinde = Methoden.abfrage_gewinde(feld);
            
            //Abfrage der Schraubenlänge
            int laenge = Methoden.abfrage_laenge();

            //Abfrage der Gewindelängen
            int gewindelaenge = Methoden.abfrage_gewindelaenge(laenge);

            //Abfrage der Gewindeart (FG/SG)
            string gewindeart = Methoden.abfrage_gewindeart();
            
            //Abfrage der Schraubenanzahl
            int menge = Methoden.abfrage_menge();

            //Abfrage der Festigkeitsklasse 
            string Fk = Methoden.abfrage_festigkeit(material);

            Console.Clear();    //Konsole bereinigen

            //Berechnung der Werte
            //Volumen berechnen
            double vol = Methoden.Volumen(typ, laenge, gewindelaenge, gewinde);

            //Ausgabe der Eingabewerte
            string a = Methoden.ausgabe_material(material);  
            string b = Methoden.ausgabe_festigkeitsklasse(Fk);
            string c = Methoden.ausgabe_schraubenkopf(typ);
            string d = Methoden.ausgabe_gewindeart(gewindeart);
            Console.WriteLine("\nGewählte Schraube: " + a + " " + b + " " + c + " " + d + " " + gewinde + "x" + laenge + "mm mit " + gewindelaenge + "mm Gewinde\n");

            //Masse berechnen
            double dichte, masse; //Einheit 
            dichte = Methoden.dichte_abfrage(material);
            masse = Methoden.masse_berechnen(dichte, vol);
            Methoden.geometrie(gewinde, gewindelaenge, laenge,gewindeart,typ,vol,dichte, menge);

            //Preis berechnen und ausgeben
            Methoden.Preis(material, gewindeart, gewindelaenge, laenge, typ, masse, menge);
